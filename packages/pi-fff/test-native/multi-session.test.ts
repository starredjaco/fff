...
import { binaryExists, FileFinder } from "@ff-labs/fff-bun";

// Real-native tests for #700/#760: @ff-labs/fff-bun is NOT mocked here, unlike
// test/*. Kept in test-native/ so the SDK mocks there can never leak in.
...
describe.skipIf(!HAS_NATIVE)(
  "fff-bun: many finders share one LMDB env per path (#700/#760)",
  () => {
...
function startSession(
  ...
  return {
    ...
    find: async (pattern: string) =>
      JSON.stringify(await tools.get("fffind")?.execute("test-call", { pattern })),
    errors: () => notifications.filter((n) => n.level === "error").map((n) => n.message),
  };
}

describe.skipIf(!HAS_NATIVE)("pi-fff: in-process double activation works (#760)", () => {
  test("two sessions in one process both search against the same dbs", async () => {
    // Defends against a cached (possibly mocked) SDK from earlier test files.
    delete (globalThis as Record<string, unknown>).__fffSdkPromiseGlobal;
